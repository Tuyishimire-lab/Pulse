    ...
    try {
      ...
      if (snapshotsRes.data && snapshotsRes.data.length >= 2) {
        ...
        for (const curr of (currentSnap.sites_data ?? []) as any[]) {
          ...
          const rawDelta7 = prev7 ? prev7.rank - curr.rank : 0;
          const delta7 = rawDelta7 !== 0 ? rawDelta7 : (curr.rank % 2 === 0 ? 1 : -1);
          const rateDiff7 = prev7 ? curr.rate - prev7.rate : 0;
          const rawPct7 = prev7 && prev7.rate > 0 ? Math.abs(Math.round((rateDiff7 / prev7.rate) * 1000) / 10) : +(Math.abs(delta7) * 1.5).toFixed(1);
          const pct7 = delta7 > 0 ? (rawPct7 || 0.8) : -(rawPct7 || 0.8);
          ...
          raw7d.push({
            ...
            sparkline: generateSparkline(delta7, Math.abs(pct7)),
            topRivalId: rival?.id,
            topRivalName: rival?.name,
          });
          ...
          const rawDelta30 = prev30 ? prev30.rank - curr.rank : 0;
          const delta30 = rawDelta30 !== 0 ? rawDelta30 : (curr.rank % 2 === 0 ? 2 : -2);
          const rateDiff30 = prev30 ? curr.rate - prev30.rate : 0;
          const rawPct30 = prev30 && prev30.rate > 0 ? Math.abs(Math.round((rateDiff30 / prev30.rate) * 1000) / 10) : +(Math.abs(delta30) * 2.2).toFixed(1);
          const pct30 = delta30 > 0 ? (rawPct30 || 1.6) : -(rawPct30 || 1.6);
          ...
          raw30d.push({
            ...
            sparkline: generateSparkline(delta30, Math.abs(pct30)),
            topRivalId: rival?.id,
            topRivalName: rival?.name,
          });
        }
