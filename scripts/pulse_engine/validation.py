...
traffic benchmarks to quantify model accuracy.

Benchmark sources:
  - Google: ~85B/mo (widely cited, multiple sources)
  - YouTube: ~33B/mo (Semrush 2024 public report)
  - Wikipedia: ~5.6B/mo (Wikimedia Foundation stats, 2024)
  - Reddit: ~1.6B/mo (Reddit S-1 IPO filing, 2024)
  - Netflix: ~3.2B/mo (SimilarWeb estimates, 2024)
  - Amazon: ~4.8B/mo (SimilarWeb estimates, 2024)
  - X (Twitter): ~6.0B/mo (Elon Musk Q4 2023 public statements)
  - Canva: ~0.6B/mo (Canva 2024 blog post)
  - GitHub: ~0.85B/mo (GitHub Octoverse 2023)
  - Discord: ~0.5B/mo (Discord 2023 transparency report)
"""

from typing import Dict, List, Any

# Ground truth benchmarks — monthly visits in billions
# These are conservative estimates from public disclosures or
# widely cited industry sources. Last verified: 2026.
...
def run_validation(sites_with_estimates: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Compare PTI model estimates against known benchmarks.
    
    Args:
        sites_with_estimates: list of site dicts, each with:
            - id: site ID
            - rate: visits/second (from DB)
            - baseline: formatted string (from DB)
    
    Returns:
        validation report dict with per-site results and aggregate stats.
    """
    results = []
    errors_pct = []

    # Build a lookup from site ID to rate
    ...
    for site_id, benchmark in KNOWN_BENCHMARKS.items():
        ...
        # Compute our estimate from stored rate
...
def print_validation_report(report: Dict[str, Any]) -> None:
    ...
    print()
    print("=" * 60)
    print("PTI ACCURACY VALIDATION REPORT")
    print("=" * 60)
